...
getMetrics returns 503 with a UI message "Scheduler unreachable. Is it healthy?" rather than a blank chart
ConfigMap missing
Initial GET returns 404; UI shows "Config not found. Is Volcano installed in the volcano-system?"
Concurrent edit
Optimistic lock via resourceVersion; 409 surfaces a diff prompt
Pod restart mid-stream
SSE handler emits a restart event, client transparently reconnects with tailLines: 0
Agent DaemonSet selector
name: volcano-agent matches multiple pods; UI shows a sub-selector by node
Invalid YAML in Save
Zod parse fails client-side before reaching the API
Plugin not in curated schema
Generic JSON-key/value editor with a "schema not curated yet" hint
Metrics output >10MB
Parse stream incrementally rather than buffering whole response


Testing strategy (high-level: details in Section 8)
Unit tests on the Prometheus parser, the YAML round-trip, and the Zod schemas. Integration tests on each tRPC procedure against a kind cluster running Volcano. Playwright e2e for the Config save flow. Manual mentor review for the typed-argument UX.
6. Technical Implementation Plan
6.1 schedulerRouter: five tRPC procedures
Path: packages/trpc/server/router/scheduler/scheduler.ts

export const schedulerRouter = router({
    // ─── Scheduler Configuration ─────────────────────────────
    getConfig: procedure.query(getConfig),
