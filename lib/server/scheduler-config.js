...
function extractFlow(config) {
    const actions = extractActions(config).map((name, index) => ({
        ...
        resultSuccess:
            actionDescriptions[name]?.resultSuccess || "Completed",
        resultFailure:
            actionDescriptions[name]?.resultFailure || "Deferred",
    }));
    ...
    const plugins = extractPlugins(config).map((plugin) => {
        const hooks = pluginCapabilities[normalizePluginName(plugin.name)] || [];
    ...
    const stepsByAction = Object.fromEntries(
        actions.map((action) => {
            const steps = (hooksByAction[action.name] || []).map((hook, index) => ({
                order: index + 1,
                hook,
                label: `Step ${index + 1}`,
